...
class NER(object):
    ...
    def init_placeholders(self):
        ...
        # 实际训练的解码器目标，实际上是 decoder_inputs
        self.decoder_targets_train = self.decoder_inputs

        self.softmax_target = tf.placeholder(
            dtype=tf.int32,
            shape=(self.batch_size,),
            name='softmax_target'
        )
    ...
    def build_decoder_crf(self):
        ...
        with tf.variable_scope('decoder_crf'):
            ...
            softmax_encoder_w = tf.get_variable(
                'softmax_encoder_w',
                [hidden_units * self.max_decode_step, hidden_units],
                initializer=initializer
            )
            softmax_encoder_b = tf.get_variable(
                'softmax_encoder_b', [hidden_units],
                initializer=tf.zeros_initializer()
            )

            softmax_encoder = tf.tanh(
                tf.nn.xw_plus_b(
                    tf.reshape(encoder_outputs, shape=[self.batch_size, -1]),
                    softmax_encoder_w, softmax_encoder_b,
                    name='softmax_encoder'
                )
            )

            softmax_decoder_w = tf.get_variable(
                'softmaxsoftmax_decoder_w_w',
                [self.target_vocab_size * self.max_decode_step, hidden_units],
                initializer=initializer
            )
            softmax_decoder_b = tf.get_variable(
                'softmax_decoder_b', [hidden_units],
                initializer=tf.zeros_initializer()
            )

            softmax_decoder = tf.tanh(
                tf.nn.xw_plus_b(
                    tf.reshape(outputs, shape=[self.batch_size, -1]),
                    softmax_decoder_w, softmax_decoder_b,
                    name='softmax_decoder'
                )
            )
            
            softmax_w = tf.get_variable('softmax_w',
                                        [hidden_units, self.n_target],
                                        initializer=initializer)
            softmax_b = tf.get_variable('softmax_b', [self.n_target],
                                        initializer=tf.zeros_initializer())
            
            softmax_logits = tf.nn.xw_plus_b(
                softmax_encoder + softmax_decoder,
                softmax_w, softmax_b,
                name='softmax_logits'
            )
            ...
            self.softmax_output = tf.nn.softmax(softmax_logits)
            ...
            onehot_labels = tf.one_hot(
                indices=self.softmax_target,
                depth=self.n_target
            )
            self.softmax_loss = tf.losses.softmax_cross_entropy(
                onehot_labels=onehot_labels,
                logits=softmax_logits
            )
            # self.softmax_loss = tf.reduce_sum(tf.reduce_sum(self.softmax_loss))
    ...
    def train(self, sess, encoder_inputs, encoder_inputs_length,
              decoder_inputs, decoder_inputs_length, softmax_target):
        """训练模型"""
        ...
        input_feed[self.softmax_target] = softmax_target
        # 设置 dropout
    ...
    def predict(self, sess,
        ...
        encoder_inputs_crf = []
        for item in encoder_inputs:
            if len(item) > self.max_decode_step:
                x = list(item)[:self.max_decode_step]
            elif len(item) < self.max_decode_step:
                x = list(item) + [WordSequence.PAD] * (self.max_decode_step - len(item))
            else: # len(item) == self.max_decode_step
                x = list(item)
            encoder_inputs_crf.append(x)
        encoder_inputs = np.array(encoder_inputs_crf)
        ...
        # crf mode
        if self.crf_loss:
            pred, softmax = sess.run([self.viterbi_sequence, self.softmax_output], input_feed)
            preds = []
